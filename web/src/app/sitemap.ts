...
import { DEPARTAMENTO_SLUG } from '@/lib/types';
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base: MetadataRoute.Sitemap = [
    ...
    ...Object.values(DEPARTAMENTO_SLUG).map((slug) => ({
  ...
  const data = await fetchAnuncios({ limit: 100 });
  const anuncios: MetadataRoute.Sitemap = (data?.items ?? []).map((a) => ({
