...
export function Reviews({
  ...
  const load = useCallback(() => {
    api<ReviewsResponse>(`/reviews?empleadorId=${empleadorId}`)
      .then(setData)
      .catch(() => setData(null));
  }, [empleadorId]);
  ...
  const propia = user
    ? data?.items.find((r) => r.autorId === user.id)
    : undefined;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      await api<Review>('/reviews', {
        method: 'POST',
        body: JSON.stringify({ empleadorId, rating, comentario }),
      });
      setEnviada(true);
      load();
    } catch (err) {
  ...
  const puedeReseñar = user?.role === 'TRABAJADOR';

  return (
    <section className="space-y-3 border-t border-gray-100 pt-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-700">
          Reseñas de {empleadorNombre}
        </h2>
        {data && data.total > 0 && (
          <p className="text-sm text-gray-600">
            <Stars value={Math.round(data.promedio ?? 0)} />{' '}
      ...
      {puedeReseñar && (
        <form onSubmit={onSubmit} className="space-y-3 rounded-md border border-gray-200 p-3">
          <p className="text-sm font-medium text-gray-700">
            {propia ? 'Editar tu reseña' : 'Calificar a este empleador'}
          </p>
          ...
          <Button type="submit" disabled={saving}>
            {saving ? 'Enviando...' : propia ? 'Actualizar reseña' : 'Enviar reseña'}
          </Button>
        </form>
      )}
    </section>
